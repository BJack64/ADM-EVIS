...
{
    public class FakturPajaks : ApplicationCollection<FakturPajak, SpBase>
    {
...
WHERE fp.IsDeleted = 0 AND fp.FgPengganti = '0' AND (fp.NoFakturPajak = @NoFakturPajak OR fp.FormatedNoFaktur = @NoFakturPajak)");
		...
		, CAST(1 AS int) AS TotalItems
FROM	dbo.View_FakturPajak fp
WHERE fp.IsDeleted = 0 AND (fp.NoFakturPajak = @NoFakturPajak OR fp.FormatedNoFaktur = @NoFakturPajak)");
            sp.AddParameter("NoFakturPajak", nomorFaktur);
            var d = GetApplicationObject(sp);
            return d;
        }
...
OPTION (OPTIMIZE FOR UNKNOWN)", filter.SortColumnName, sortOrder));
        ...
        public static FakturPajak Save(FakturPajak data)
        {
            ...
            {
                sp = new SpBase(@"UPDATE [dbo].[FakturPajak] WITH (ROWLOCK, UPDLOCK)
                                           SET
                                           [FCode] = @FCode 
                                          ...
                                          ,[JumlahHargaJual] = @JumlahHargaJual
                                          ,[IsCoretax] = @IsCoretax
                                          ,[StatusApproval] = @StatusApproval
                                          ,[StatusFaktur] = (CASE WHEN @StatusFaktur IS NOT NULL THEN @StatusFaktur ELSE [StatusFaktur] END)
                                          ...
                                          ,[Referensi] = @Referensi
                                          ,[ObjectID] = (CASE WHEN @ObjectID IS NOT NULL AND @ObjectID = 0 THEN NULL ELSE @ObjectID END)
            ...
            {
                sp = new SpBase(@"
                    IF EXISTS(SELECT * FROM FakturPajak fp WITH (NOLOCK) WHERE LTRIM(RTRIM(LOWER(fp.[UrlScan]))) = LTRIM(RTRIM(LOWER(@UrlScan))) AND IsDeleted = 0 AND Created >= CONVERT(DATE, Dateadd(month, -24, Getdate())))
                    BEGIN
	                    SET @outmsgs = (
		                    SELECT	CONCAT('Sudah discan pada ' , CONVERT(varchar, fp.Created, 13) , ' oleh ' 
                    ...
                    BEGIN
	                    ...
	                    INSERT INTO [dbo].[FakturPajak]
                         ...
                         VALUES
                               ...
                               ,@StatusApproval
                               ,(CASE 
                                    WHEN @StatusFaktur IS NULL OR (@StatusFaktur IS NOT NULL AND @StatusFaktur = '') THEN 
                                        (CASE WHEN ISNULL((SELECT TOP 1 ISNULL(IsByPass,0) FROM LogPostingTanggalLaporan WHERE [Url] = LTRIM(RTRIM(@UrlScan))),0) = 1 THEN 'Pending Verification' ELSE @StatusFaktur END)
                                    ELSE @StatusFaktur
                                END)
                               ,@Dikreditkan
                               ,@MasaPajak
                               ...
                               ,@Status
                               ,@Referensi
                               ,(CASE WHEN @ObjectID IS NOT NULL AND @ObjectID = 0 THEN NULL ELSE @ObjectID END)
                               ,@CertificateID
                               ,@StatusPayment
                               ,@Remark
                               ,@Source
                               ,@IsOutstanding
                               ,@IsCreatedCSV
                               ,ISNULL((SELECT TOP 1 ISNULL(IsByPass,0) FROM LogPostingTanggalLaporan WHERE [Url] = LTRIM(RTRIM(@UrlScan))),0)
                              ,@IsDeleted
                               ); SELECT @FakturPajakId = @@IDENTITY; END");
            ...
            sp.AddParameter("JumlahHargaJual", data.JumlahHargaJual.HasValue ? data.JumlahHargaJual.Value : SqlDecimal.Null);
            sp.AddParameter("IsCoretax", data.IsCoretax.HasValue ? data.IsCoretax.Value : SqlBoolean.Null);
        ...
        public static FakturPajak SaveCoretax(FakturPajak data)
        {
            data.WasSaved = false;
            data.UIMessage = string.Empty;
            SpBase sp;
            ...
            if (data.FakturPajakId > 0)
            {
                sp = new SpBase(@"UPDATE [dbo].[FakturPajak] WITH (ROWLOCK, UPDLOCK)
                                          ...
                                          --,[FormatedNpwpPenjual] = dbo.FormatNpwp(@NPWPPenjual)
                                          --,[FormatedNpwpLawanTransaksi] = dbo.FormatNpwp(@NPWPLawanTransaksi)
                                          ,[FormatedNoFaktur] = @NoFakturPajak
                                          ,[FormatedNpwpPenjual] = @NPWPPenjual
                                          ,[FormatedNpwpLawanTransaksi] = @NPWPLawanTransaksi
                                          ,[Referensi] = @Referensi
                                          ,[ObjectID] = (CASE WHEN @ObjectID IS NOT NULL AND @ObjectID = 0 THEN NULL ELSE @ObjectID END)
                                          ,[CertificateID] = @CertificateID
                                          ,[StatusPayment] = @StatusPayment
            ...
            {
                sp = new SpBase(@"
                    IF EXISTS(SELECT TOP 1 * FROM FakturPajak WITH (ROWLOCK, UPDLOCK) WHERE [NoFakturPajak] = @NoFakturPajak AND [IsDeleted] = 0)
                    BEGIN
	                    SET @outmsgs = (
		                    SELECT	TOP 1 CONCAT('Sudah discan pada ' , CONVERT(varchar, fp.Created, 13) , ' oleh ' 
				                    , fp.CreatedBy , '. No FP ' , fp.FormatedNoFaktur , ' sudah ada di Masa Pajak ' , fp.MasaPajakName , ' ' , CAST(fp.TahunPajak AS varchar(4))
				                    , ', Nomor Filling Index ' , fp.FillingIndex + ' (Save)')
		                    FROM	View_FakturPajak fp 
		                    WHERE	[NoFakturPajak] = @NoFakturPajak AND IsDeleted = 0
	                    )
                    END
                    ELSE
                    BEGIN
     ...
     VALUES
        ...
        {
            var sp = new SpBase(string.Format(@"UPDATE dbo.FakturPajak WITH (ROWLOCK, UPDLOCK) SET IsDeleted = 1, Modified = GETDATE(), ModifiedBy = @ModifiedBy WHERE FakturPajakId = @FakturPajakId;"));
            sp.AddParameter("FakturPajakId", fakturPajakId);
        ...
        {
            var sp = new SpBase(string.Format(@"UPDATE dbo.FakturPajak WITH (ROWLOCK, UPDLOCK) SET IsDeleted = 1, Modified = GETDATE(), ModifiedBy = @ModifiedBy 
WHERE FakturPajakId IN (SELECT Data FROM dbo.Split(@FakturPajakIds))"));
        ...
        public static bool UpdateStatusById(long fakturPajakId, ApplicationEnums.StatusFakturPajak eStatus, string modifiedBy)
        {
            ...
            var sp = new SpBase(@"UPDATE FakturPajak WITH (ROWLOCK, UPDLOCK)
SET [Status] = @Status
	,Modified = GETDATE()
	,ModifiedBy = @ModifiedBy
WHERE	FakturPajakId = @fakturPajakId");
...
WHERE fp.IsDeleted = 0 AND fp.NoFakturPajak = @NoFakturPajak AND fp.StatusFaktur='Faktur Diganti' ");
        ...
        public static int CheckNoFPKhusus(string NoFakturPajak)
        {
            var sp = new SpBase(string.Format(@"SELECT COUNT(NoFakturPajak) AS NoFakturPajak FROM FakturPajak WHERE FormatedNoFaktur = ISNULL(@NoFakturPajak,'') and IsDeleted=0"));
            sp.AddParameter("NoFakturPajak", NoFakturPajak);
        ...
        public static string GetCreateDateNoFPKhusus(string NoFakturPajak)
        {
            var sp = new SpBase(string.Format(@"SELECT Created AS CreateDate FROM FakturPajak WHERE FormatedNoFaktur = @NoFakturPajak and IsDeleted=0"));
            sp.AddParameter("NoFakturPajak", NoFakturPajak);
            
            
            var data = sp.ExecuteScalar();
