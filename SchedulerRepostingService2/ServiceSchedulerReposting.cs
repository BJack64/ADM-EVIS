...
namespace SchedulerRepostingService
{
    public partial class ServiceSchedulerReposting : ServiceBase
    {
        ...
        void timer1_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            {
                Logger.WriteLog(out logKey, LogLevel.Error, "Error Timer_Elapsed with message :" + ex.Message, MethodBase.GetCurrentMethod(), ex);
            }
        ...
        private static Task TaskHitDJP(long Id, string Source, string Url, string FPdjpID, string CreatedBy, string tmp_file)
        {
            var msgError = "";
            var logKey = "";
            string eStatus = "";
            if (Source.ToLower() == "delvi")
            {
                ...
                ValidateFakturLib.ValidateFakturObjectAPI_2(Url, Source, CreatedBy, FPdjpID, tmp_file, out logKey, out eStatus, out msgError);
                ...
                LogPostingTanggalLaporans.SetStatus(Id, aa, msgError);
            }
            return Task.CompletedTask;
        }
        ...
        private static void StartProcessTask()
        {
            ...
            if (TimeSpan.Compare(timeConfig, timeCurrent) == 0)
            {
                ...
                {
                    var data = getData[i];
                    listOfTask.Add(TaskHitDJP(data.Id, data.Source, data.Url, data.FPdjpID, data.CreatedBy, @"D:\xmlReposting.xml"));
                }
